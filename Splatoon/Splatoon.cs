...
public unsafe class Splatoon : IDalamudPlugin
{
    ...
    static bool CheckActorIsAlive(Element e, GameObject a)
    {
        if (!(a is Character)) return true;
        var isAlive = ((Character) a).CurrentHp > 0;
        if (e.refActorIsAlive == 1 && !isAlive) return false;
        if (e.refActorIsAlive == 2 && isAlive) return false;
        return true;
    }

    bool CheckActorHuntTargetRank(Element e, GameObject a)
    {
        if (e.refActorHuntTargetRank == 0) return true;
        if (!(a is BattleNpc)) return false;
        if (!HuntTargetToRank.TryGetValue(((BattleNpc)a).DataId, out var rank)) return false;
        ...
        if (e.overlayText.Length > 0)
        {
            ...
            if (go != null)
            {
                text = text
                    ...
                    .Replace("$CLASSJOB", $"{(go is Character chrx ? chrx.ClassJob.GameData.Abbreviation : "")}");
            }
